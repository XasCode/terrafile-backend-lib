...
const useFsHelpers = fsHelpers.use(fsHelpers.mock, [
  ...
  `./__tests__/modules/test-module/main.tf`,
...
const { getAbsolutePath, createDir, touchFile, rimrafDirs, checkIfFileExists } = useFsHelpers;

const testDirs = [`err_vendor1`, `err_vendor2`, `err_vendor3`, `err_vendor4`, `err_vendor_lerror`, `err_vendor_2x`];

describe(`read file contents should read specified json file and validate its contents`, async () => {
  beforeAll(async () => {
    await rimrafDirs(testDirs);
  });
  ...
  it(`should successfully read a valid terrafile when provided a relative path`, async () => {
    ...
    const testJson = await JSON.parse(readFileSync(getAbsolutePath(configFile).value, `utf-8`));
  ...
  it(`should successfully read a valid terrafile when provided an absolute path`, async () => {
  ...
  it(`ensure our sample file passes tests`, async () => {
  ...
  it(`should err on lack read access to file`, async () => {
  ...
  it(`should err on bad local dir`, async () => {
  ...
  it(`should err on copy 2x`, async () => {
